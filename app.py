...
* { box-sizing: border-box; margin: 0; padding: 0; }
html, body, [class*="css"] { font-family: 'Inter', sans-serif; background: #ffffff; color: #111827; }
.block-container { padding: 0 2rem 3rem; max-width: 1100px; }
.stApp, [data-testid="stAppViewContainer"], [data-testid="stHeader"] { background:#ffffff !important; }
.stMarkdown p, label { color:#111827 !important; }
.navbar { background:#fff; border-bottom:1px solid #e5e7eb; padding:0.7rem 0; margin-bottom:1.4rem; display:flex; align-items:center; justify-content:space-between; }
.nav-logo { font-size:1.1rem; font-weight:800; color:#111827; letter-spacing:-0.02em; }
.nav-logo span { color:#2563eb; }
.nav-links { display:flex; gap:1.4rem; }
.nav-link { font-size:0.76rem; color:#6b7280; font-weight:500; }
.breadcrumb { font-size:0.68rem; color:#9ca3af; margin-bottom:1rem; }
.breadcrumb span { color:#111827; font-weight:600; }
.product-header { border:1px solid #e5e7eb; border-radius:10px; padding:1rem 1.2rem; background:#fff; display:flex; align-items:flex-start; justify-content:space-between; gap:1.5rem; margin-bottom:1rem; }
.product-left { flex:1; min-width:0; }
.product-type-pill { display:inline-block; background:#f0f9ff; color:#0369a1; font-size:0.6rem; font-weight:700; padding:0.12rem 0.45rem; border-radius:20px; text-transform:uppercase; letter-spacing:0.07em; margin-bottom:0.4rem; }
.product-name-link { font-size:1.05rem; font-weight:800; color:#111827; line-height:1.3; text-decoration:none; display:block; margin-bottom:0.2rem; }
.product-name-link:hover { color:#2563eb; text-decoration:underline; }
.product-meta { font-size:0.72rem; color:#6b7280; margin-bottom:0.75rem; }
.product-meta strong { color:#374151; }
.price-row { display:flex; align-items:baseline; gap:0.35rem; flex-wrap:wrap; margin-bottom:0.55rem; }
.price-main { font-size:1.3rem; font-weight:800; color:#111827; }
.price-orig { font-size:0.82rem; color:#9ca3af; text-decoration:line-through; }
.price-off  { font-size:0.7rem; font-weight:700; color:#fff; background:#16a34a; padding:0.1rem 0.35rem; border-radius:4px; }
.stock-badge { display:inline-flex; align-items:center; gap:0.25rem; font-size:0.68rem; font-weight:600; color:#16a34a; margin-bottom:0.7rem; }
.stock-dot { width:5px; height:5px; background:#16a34a; border-radius:50%; }
.param-grid { display:grid; grid-template-columns:repeat(2,1fr); gap:0.3rem; }
.param-box { border:1px solid #e5e7eb; border-radius:6px; padding:0.38rem 0.55rem; background:#fafafa; }
.param-label { font-size:0.55rem; color:#9ca3af; text-transform:uppercase; letter-spacing:0.05em; }
.param-value { font-size:0.76rem; font-weight:600; color:#111827; margin-top:0.06rem; }
.feat-section { margin-top:0.7rem; }
.feat-label { font-size:0.57rem; font-weight:700; text-transform:uppercase; letter-spacing:0.09em; color:#9ca3af; margin-bottom:0.35rem; }
.feat-item { display:flex; align-items:flex-start; gap:0.35rem; margin-bottom:0.22rem; }
.feat-check { color:#16a34a; font-size:0.72rem; flex-shrink:0; margin-top:1px; }
.feat-text { font-size:0.72rem; color:#374151; line-height:1.4; }
.product-right { flex:0 0 auto; display:flex; flex-direction:column; align-items:flex-end; gap:0.5rem; }
.sku-tag { font-size:0.58rem; color:#9ca3af; font-weight:500; }
.view-btn { display:inline-flex; align-items:center; gap:0.35rem; background:#111827; color:#fff; font-size:0.72rem; font-weight:700; padding:0.45rem 0.9rem; border-radius:7px; text-decoration:none; white-space:nowrap; }
.view-btn:hover { background:#2563eb; }
.divider { border:none; border-top:1px solid #e5e7eb; margin:1.2rem 0; }
.toggle-label { font-size:0.85rem; font-weight:700; color:#111827; margin-bottom:0.55rem; }
.desc-box { border:1px solid #e5e7eb; border-radius:10px; padding:1rem 1.2rem; background:#fff; margin-top:0.75rem; }
.desc-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:0.75rem; }
.desc-title { font-size:0.6rem; font-weight:800; text-transform:uppercase; letter-spacing:0.12em; color:#7c3aed; }
.desc-count { font-size:0.6rem; color:#9ca3af; }
.rfy-row { display:flex; align-items:flex-start; gap:0.38rem; margin-bottom:0.32rem; }
.rfy-arrow { color:#7c3aed; font-size:0.68rem; margin-top:2px; flex-shrink:0; }
.rfy-text { font-size:0.74rem; color:#374151; line-height:1.45; }
.rfy-text strong { color:#111827; }
.rfy-text em { color:#6b7280; font-style:normal; }
.faq-eyebrow { font-size:0.58rem; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:#2563eb; margin-bottom:0.25rem; }
.faq-main-title { font-size:0.9rem; font-weight:800; color:#111827; margin-bottom:0.18rem; }
.faq-subtitle { font-size:0.65rem; color:#4b5563; line-height:1.45; margin-bottom:0.55rem; }
.faq-a { font-size:0.68rem; color:#374151; line-height:1.55; padding-bottom:0.4rem; }
.faq-a strong { color:#111827; }
.footer { border-top:1px solid #e5e7eb; padding-top:1rem; margin-top:2rem; display:flex; justify-content:space-between; align-items:center; }
.footer-text { font-size:0.65rem; color:#9ca3af; }
#MainMenu, footer, header { visibility:hidden; }
div[data-testid="stExpander"] { border:1px solid #e5e7eb !important; border-radius:7px !important; background:#fff !important; margin-bottom:0.28rem !important; box-shadow:none !important; overflow:hidden !important; }
div[data-testid="stExpander"] summary { font-size:0.72rem !important; font-weight:700 !important; color:#0f172a !important; padding:0.38rem 0.55rem !important; background:#fff !important; }
div[data-testid="stExpander"] summary:hover { color:#2563eb !important; background:#f8fafc !important; }
div[data-testid="stExpander"] summary p { color:#0f172a !important; font-size:0.72rem !important; font-weight:700 !important; }
div[data-testid="stExpanderDetails"] { background:#fff !important; padding:0 0.55rem 0.4rem !important; }
div[data-testid="stExpanderDetails"] p { color:#374151 !important; font-size:0.68rem !important; line-height:1.55 !important; }
div[data-testid="stExpander"] svg { color:#334155 !important; fill:#334155 !important; }
div[data-testid="stButton"] > button { font-size:0.76rem !important; padding:0.4rem 0.8rem !important; border-radius:7px !important; }
</style>
""", unsafe_allow_html=True)
# ── Load ─────────────────────────────────────────────────────────
@st.cache_resource(show_spinner=False)
...
def norm_name(s):
    """Normalize a product name for duplicate detection (case/space/punct insensitive)."""
...
def _price_ok(c, a, band=PRICE_BAND):
    try:
        ...
        if ap <= 0 or cp <= 0: return False
        return ap * (1 - band) <= cp <= ap * (1 + band)
    except: return False
# ── Search ────────────────────────────────────────────────────────
def strict_ok(c, a, mode):
    if not avail(c): return False
    if c["product_id"] == a["product_id"]: return False
    if norm_name(c["product_name"]) == norm_name(a["product_name"]): return False  # block name-duplicate variants
    if c["product_type"] != a["product_type"]: return False
    if c["gender"] != a["gender"]: return False
    band = SAME_PRICE_BAND if mode == "same" else PRICE_BAND
    if not _price_ok(c, a, band=band): return False
    if mode == "same" and c["brand"] != a["brand"]: return False
    if mode == "diff" and c["brand"] == a["brand"]: return False
    return True
def relaxed_ok(c, a, mode):
    """Fallback: same brand is enough — type constraint already failed"""
    if not avail(c): return False
    if c["product_id"] == a["product_id"]: return False
    if norm_name(c["product_name"]) == norm_name(a["product_name"]): return False  # block name-duplicate variants
    band = SAME_PRICE_BAND if mode == "same" else 0.50
    if not _price_ok(c, a, band=band): return False
    if mode == "same" and c["brand"] != a["brand"]: return False
    if mode == "diff" and c["brand"] == a["brand"]: return False
    return True
def find_alts(aidx, mode):
    ...
    scores, ids = index.search(vec, SEARCH_K)
    results, brands = [], defaultdict(int)
    seen_names = {norm_name(anchor["product_name"])}
    seen_ids   = {anchor["product_id"]}
    # Pass 1 — strict (same type + gender + price band)
    for score, rid in zip(scores[0], ids[0]):
        if len(results) >= MAX_ALTS: break
        if rid < 0 or rid >= len(records): continue
        c = records[int(rid)]
        if c["product_id"] in seen_ids: continue
        if not strict_ok(c, anchor, mode): continue
        nn = norm_name(c["product_name"])
    ...
    # Pass 2 — relaxed (same brand any type, or any brand gender match)
    for score, rid in zip(scores[0], ids[0]):
        if len(results) >= MAX_ALTS: break
        if rid < 0 or rid >= len(records): continue
        c = records[int(rid)]
        ...
        seen_names.add(nn)
        seen_ids.add(c["product_id"])
    # Final hard safety net: de-duplicate by normalized name AND drop anchor if it ever slipped in
    final, final_names = [], set()
    for c, fb in results:
        nn = norm_name(c["product_name"])
        if c["product_id"] == anchor["product_id"]: continue
        if nn == norm_name(anchor["product_name"]): continue
        if nn in final_names: continue
        final.append((c, fb))
        final_names.add(nn)
    results = final[:MAX_ALTS]
    return results, any(r[1] for r in results)
# ── LLM: which is right for you ──────────────────────────────────
def get_rfy(anchor, alts, mode):
    cache_key = f"rfy6_{anchor['product_id']}_{mode}_{len(alts)}"
    if cache_key in db: return db[cache_key]
    anchor_norm = norm_name(anchor["product_name"])
    alt_norms   = [norm_name(c["product_name"]) for c, _ in alts]
    def fallback():
        out = [{"scenario": "you want to stick with your original pick",
                "product": anchor["product_name"],
                "reason": "your selected product"}]
        for c, _ in alts:
            note, _ = price_diff_label(anchor["price"], c["price"])
            feats = feat_lines(c.get("key_features", ""), 1)
            out.append({
                "scenario": feats[0][:55] if feats else f"you prefer {c['brand']}",
                "product": c["product_name"],
                "reason": note or "similar specs"
            })
        return out
    if not groq_client: return fallback()
    cands_text = "\n".join([
        f"Alt {i+1}: {c['product_name']} · {c['brand']} · {money(c['price'])} · {short(c.get('key_features',''), 150)}"
        for i, (c, _) in enumerate(alts)
    ])
    if mode == "same":
        mode_ctx = f"same-brand lineup — ALL alternatives are from {anchor['brand']}"
        brand_note = f"Note: all products shown are from {anchor['brand']} — do NOT say 'different brand' anywhere."
    else:
        mode_ctx = "cross-brand alternatives — all from different brands"
        brand_note = f"Note: all alternatives are from brands OTHER than {anchor['brand']}."
    prompt = f"""You are a product advisor at APeak outdoor store.
Write a "Which is right for you?" guide for {mode_ctx}.
SELECTED (the anchor product — this is bullet #1 ONLY, never use it again later): {anchor['product_name']} · {anchor['brand']} · {money(anchor['price'])}
Features: {short(anchor.get('key_features',''), 200)}
ALTERNATIVES — there are exactly {len(alts)} of them. Each must appear EXACTLY ONCE, in bullets #2 through #{len(alts)+1}. Do not invent any other product names.
{cands_text}
{brand_note}
Output exactly {len(alts)+1} bullets total, in this strict order:
- Bullet 1: about {anchor['product_name']} (the anchor)
- Bullets 2..{len(alts)+1}: one each for the {len(alts)} alternatives listed above, each used exactly once
Each bullet:
- scenario: starts "you want" (never "if you want") — max 8 words — specific real need or feature
- product: exact product name copied verbatim from above
- reason: max 7 words — real spec or price difference — always use the $ sign for currency, NEVER write "Rs."
Return ONLY valid JSON array, no markdown:
[{{"scenario":"you want...","product":"name","reason":"spec or price"}}]"""
    try:
        resp = groq_client.chat.completions.create(
            model=GROQ_MODEL,
            messages=[
                {"role": "system", "content": "Return valid JSON array only. No markdown. No preamble. Always use the $ sign for currency — never write Rs. Never repeat a product. Never invent product names not given to you."},
                {"role": "user", "content": prompt}
            ],
        ...
        if "[" in raw and "]" in raw:
            raw = raw[raw.index("["):raw.rindex("]")+1]
        result = json.loads(raw)
        # sanitize: replace any stray "Rs." with $
        for item in result:
            if isinstance(item.get("reason"), str):
                item["reason"] = item["reason"].replace("Rs. ", "$").replace("Rs.", "$")
            if isinstance(item.get("scenario"), str):
                item["scenario"] = item["scenario"].replace("Rs. ", "$").replace("Rs.", "$")

        # ── STRICT structural validation ──
        # 1. find the anchor bullet (must match anchor name) — take the FIRST one only
        anchor_bullet = None
        alt_bullets_by_norm = {}
        for item in result:
            pname = norm_name(item.get("product", ""))
            if pname == anchor_norm and anchor_bullet is None:
                anchor_bullet = item
            elif pname in alt_norms and pname not in alt_bullets_by_norm:
                alt_bullets_by_norm[pname] = item
            # anything else (hallucinated names, duplicate anchor mentions, duplicate alts) is dropped

        # 2. require we have the anchor + every single alt, no exceptions
        if anchor_bullet is None or len(alt_bullets_by_norm) != len(alts):
            return fallback()

        # 3. rebuild in correct order: anchor first, then alts in their original `alts` order
        final = [anchor_bullet]
        for nn in alt_norms:
            final.append(alt_bullets_by_norm[nn])
...
anchor = records[sel_idx]
st.markdown(
    f'<div class="breadcrumb">Home · {anchor["main_category"]} · {anchor["product_type"]} · <span>{anchor["product_name"]}</span></div>',
    unsafe_allow_html=True
)
# ── PRODUCT HEADER ────────────────────────────────────────────────
orig = anchor.get("original_price", 0)
curr = anchor.get("price", 0)
off  = pct_off(orig, curr)
cols_list = ", ".join(anchor.get("colors", [])[:4]) or "—"
qty  = int(float(anchor.get("quantity", 0) or 0))
feats = feat_lines(anchor.get("key_features", ""), 5)
product_url = anchor.get("product_url", "")
price_html = (
    '<div class="price-row">'
    f'<span class="price-main">{money(curr)}</span>'
    + (f'<span class="price-orig">{money(orig)}</span>' if off else "")
    + (f'<span class="price-off">{off}</span>' if off else "")
    + '</div>'
)
stock_html = (
    '<div class="stock-badge"><div class="stock-dot"></div>'
    f'In Stock &nbsp;·&nbsp; {qty} units</div>' if avail(anchor) else ""
)
param_html = (
    '<div class="param-grid">'
    f'<div class="param-box"><div class="param-label">Brand</div><div class="param-value">{anchor["brand"]}</div></div>'
    f'<div class="param-box"><div class="param-label">Gender</div><div class="param-value">{anchor["gender"]}</div></div>'
    f'<div class="param-box"><div class="param-label">Type</div><div class="param-value">{anchor["product_type"]}</div></div>'
    f'<div class="param-box"><div class="param-label">Colors</div><div class="param-value" style="font-size:0.68rem">{cols_list}</div></div>'
    f'<div class="param-box"><div class="param-label">SKU</div><div class="param-value" style="font-size:0.65rem">{anchor["sku"]}</div></div>'
    f'<div class="param-box"><div class="param-label">Category</div><div class="param-value" style="font-size:0.68rem">{anchor["main_category"]}</div></div>'
    '</div>'
)
feats_html = ""
if feats:
    rows = "".join(
        f'<div class="feat-item"><span class="feat-check">✓</span><span class="feat-text">{f}</span></div>'
        for f in feats
    )
    feats_html = f'<div class="feat-section"><div class="feat-label">Key Features</div>{rows}</div>'
name_html = (
    f'<a class="product-name-link" href="{product_url}" target="_blank">{anchor["product_name"]}</a>'
    if product_url else
    f'<span class="product-name-link" style="cursor:default">{anchor["product_name"]}</span>'
)
view_btn_html = (
    f'<a class="view-btn" href="{product_url}" target="_blank">View on APeak <span>↗</span></a>'
    if product_url else ""
)
st.markdown(
    '<div class="product-header">'
    + '<div class="product-left">'
    + f'<div class="product-type-pill">{anchor["product_type"]}</div>'
    + name_html
    + f'<div class="product-meta">by <strong>{anchor["brand"]}</strong> &nbsp;·&nbsp; {anchor["gender"]} &nbsp;·&nbsp; {anchor["main_category"]}</div>'
    + price_html + stock_html + param_html + feats_html
    + '</div>'
    + '<div class="product-right">'
    + f'<div class="sku-tag">SKU: {anchor["sku"]}</div>'
    + view_btn_html
    + '</div>'
    + '</div>',
    unsafe_allow_html=True
...
st.markdown('<div class="toggle-label">Compare With Best Alternatives</div>', unsafe_allow_html=True)
...
with b2:
    if st.button("🔀  Different Brand", use_container_width=True,
                 type="primary" if st.session_state.mode == "diff" else "secondary"):
        st.session_state.mode = "diff"; st.rerun()
alts, _ = find_alts(sel_idx, st.session_state.mode)
if not alts:
    st.markdown("""
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:1.2rem;text-align:center;margin-top:0.75rem;">
      <div style="font-size:1.2rem;margin-bottom:0.3rem">🔍</div>
      <div style="font-size:0.78rem;color:#374151;font-weight:600;margin-bottom:0.2rem">No alternatives found</div>
      <div style="font-size:0.7rem;color:#9ca3af">Try switching modes or selecting a different product.</div>
    </div>""", unsafe_allow_html=True)
else:
    # URL + brand maps for clickable, brand-labeled product names in RFY
    url_map   = {anchor["product_name"]: anchor.get("product_url", "")}
    brand_map = {anchor["product_name"]: anchor.get("brand", "")}
    for cand, _ in alts:
        url_map[cand["product_name"]]   = cand.get("product_url", "")
        brand_map[cand["product_name"]] = cand.get("brand", "")
    with st.spinner("Generating recommendation..."):
        rfy = get_rfy(anchor, alts, st.session_state.mode)
    mode_label = "Same Brand" if st.session_state.mode == "same" else "Different Brand"
    band_pct   = int(SAME_PRICE_BAND * 100) if st.session_state.mode == "same" else int(PRICE_BAND * 100)
    # RFY box header
    st.markdown(
        '<div class="desc-box">'
        '<div class="desc-header">'
        f'<span class="desc-title">✦ Which is right for you</span>'
        f'<span class="desc-count">{len(alts)} {mode_label} Alternative{"s" if len(alts)>1 else ""} · ±{band_pct}% price range</span>'
        '</div>',
        unsafe_allow_html=True
    )
    for item in rfy:
        sc     = item.get("scenario", "").strip()
        if sc.lower().startswith("if "): sc = sc[3:].strip()
        if sc.lower().startswith("you want "): sc = sc[9:].strip()
        sc     = sc.rstrip(".").capitalize()
        prod   = item.get("product", "")
        reason = item.get("reason", "")
        prod_url   = url_map.get(prod, "")
        prod_brand = brand_map.get(prod, "")
        brand_html = f' <span style="color:#9ca3af;font-weight:500;">({prod_brand})</span>' if prod_brand else ""
        prod_html = (
            f'<a href="{prod_url}" target="_blank" '
            f'style="color:#111827;font-weight:700;text-decoration:none;border-bottom:1px solid #d1d5db;" '
            f'onmouseover="this.style.color=\'#2563eb\'" onmouseout="this.style.color=\'#111827\'">'
            f'{prod}</a>{brand_html}'
            if prod_url else f'<strong>{prod}</strong>{brand_html}'
        )
        st.markdown(
            '<div class="rfy-row">'
            '<span class="rfy-arrow">→</span>'
            f'<span class="rfy-text">If you want {sc} → {prod_html} <em>({reason})</em></span>'
            '</div>',
            unsafe_allow_html=True
        )
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown(
        '<p style="font-size:0.58rem;color:#bbb;margin-top:0.25rem;">ⓘ Ranked on semantic similarity · stock confirmed at time of search</p>',
        unsafe_allow_html=True
    )
# ── FAQs ─────────────────────────────────────────────────────────
st.markdown('<hr class="divider">', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
faqs = [
    ("Are all alternatives in stock?",
     "Yes — only products with confirmed quantity greater than zero are shown. Stock is verified at the time of search."),
    ("How close are the prices?",
     "Same-brand alternatives allow up to <strong>±60%</strong> price range — because within the same brand, a slightly higher price is often acceptable. Different-brand alternatives stay within <strong>±30%</strong>."),
    ("Are same-brand alternatives always from the same brand?",
     "Yes, always. Same Brand mode strictly filters to show only products from the same brand as your selected item. Different Brand mode shows only products from other brands."),
    ("Can I select size or color here?",
     "Use the <strong>View on APeak ↗</strong> link to open the full product page and choose size, color, and quantity."),
    ("Why does Same Brand sometimes show fewer than 3 results?",
     "Some brands carry only a few products in a given category. We only show genuine matches — fewer high-quality results is always better than padding with unrelated items."),
    ("Can I return an alternative?",
     "APeak's standard return policy applies. Check the product page for the return window and conditions before purchasing."),
]
for q, a in faqs:
    with st.expander(q):
        st.markdown(f'<div class="faq-a">{a}</div>', unsafe_allow_html=True)
